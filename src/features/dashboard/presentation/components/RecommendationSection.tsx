...
import { Folder, MoreVertical, LayoutGrid, List, Download, Info, Star, StarOff } from 'lucide-react';
...
export function RecommendationSection({
    ...
    const downloadFolderByServiceType = useCallback(async (folder: { id: string; name: string }) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Gagal mengunduh folder');
            }
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 p-1 border-none shadow-none bg-transparent">
                    {realActivities.map((activity) => (
                        <div key={activity.id} className="group relative border border-gray-200/80 bg-white rounded-2xl p-4 flex flex-col hover:border-[#B39B7D] hover:shadow-md transition-all cursor-pointer h-[180px]">
                            <div className="flex-1 flex justify-center items-center py-2" onClick={() => {
                                const slug = activity.companyName.toLowerCase().replace(/\s+/g, '-');
                                const typeSlug = activity.service.toLowerCase().replace(/\s+/g, '-');
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                    ...
                    {realActivities.map((activity, idx) => (
                        <div key={activity.id} className={`flex items-center justify-between p-4 hover:bg-gray-50 transition-colors ${idx !== realActivities.length - 1 ? 'border-b border-gray-100' : ''}`} onClick={() => {
                                const slug = activity.companyName.toLowerCase().replace(/\s+/g, '-');
                                const typeSlug = activity.service.toLowerCase().replace(/\s+/g, '-');
                                router.push(`/services/${slug}/${typeSlug}`);
                        }}>
